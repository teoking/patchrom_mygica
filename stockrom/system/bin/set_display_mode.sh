...
#    DVB_EXIST=yes
    ...
    DVB_EXIST=no
#fi
#added by clei for other prop
#echo c:17:1 > /dev/gpio_dev
chmod 666 /dev/i2c-0
chmod 666 /sys/devices/platform/meson-remote/speed
chmod 666 /sys/class/video/screen_mode
if [ `getprop ro.platform.falsestandby` = "true" ] ; then
   chmod 666 /sys/class/amhdmitx/amhdmitx0/debug
fi 
if [ -n "`lsusb |grep 0bda:8176`" ];then
   setprop ubootenv.var.wifi rtl8192cu
   setprop persist.sys.hostap true
fi
#echo write 0 c 0x1b30 > /sys/class/amlogic/debug
busybox rm -rf /data/data/com.android.vending/cache/*
setprop ubootenv.var.has.accelerometer `getprop ro.platform.has.rotate` 

case `getprop ubootenv.var.verhw` in
	MX_DONGLE|MXS_DONGLE)
	#	setprop ro.screen.remove.tvout true #Rony remove
		setprop hw.hasethernet false
		setprop online.ip `getprop online.ip`/dongle/
		;;
esac
...
case $outputmode in 
    ...
    ;;   
    svga)
    if [ "$(getprop ubootenv.var.svgaoutputx)" = "" ] ; then 
            outputx=0
        else outputx=$(getprop ubootenv.var.svgaoutputx)
        fi
        if [ "$(getprop ubootenv.var.svgaoutputy)" = "" ] ; then 
            outputy=0
        else outputy=$(getprop ubootenv.var.svgaoutputy)
        fi
        if [ "$(getprop ubootenv.var.svgaoutputwidth)" = "" ] ; then 
            outputwidth=800
        else outputwidth=$(getprop ubootenv.var.svgaoutputwidth)
        fi
        if [ "$(getprop ubootenv.var.svgaoutputheight)" = "" ] ; then 
            outputheight=600
        else outputheight=$(getprop ubootenv.var.svgaoutputheight)    
     fi          
    ;;      
    xga)
    if [ "$(getprop ubootenv.var.xgaoutputx)" = "" ] ; then 
            outputx=0
        else outputx=$(getprop ubootenv.var.xgaoutputx)
        fi
        if [ "$(getprop ubootenv.var.xgaoutputy)" = "" ] ; then 
            outputy=0
        else outputy=$(getprop ubootenv.var.xgaoutputy)
        fi
        if [ "$(getprop ubootenv.var.xgaoutputwidth)" = "" ] ; then 
            outputwidth=1024
        else outputwidth=$(getprop ubootenv.var.xgaoutputwidth)
        fi
        if [ "$(getprop ubootenv.var.xgaoutputheight)" = "" ] ; then 
            outputheight=768
        else outputheight=$(getprop ubootenv.var.xgaoutputheight)    
     fi          
    ;;         
    sxga)
    if [ "$(getprop ubootenv.var.sxgaoutputx)" = "" ] ; then 
            outputx=0
        else outputx=$(getprop ubootenv.var.sxgaoutputx)
        fi
        if [ "$(getprop ubootenv.var.sxgaoutputy)" = "" ] ; then 
            outputy=0
        else outputy=$(getprop ubootenv.var.sxgaoutputy)
        fi
        if [ "$(getprop ubootenv.var.sxgaoutputwidth)" = "" ] ; then 
            outputwidth=1280
        else outputwidth=$(getprop ubootenv.var.sxgaoutputwidth)
        fi
        if [ "$(getprop ubootenv.var.sxgaoutputheight)" = "" ] ; then 
            outputheight=1024
        else outputheight=$(getprop ubootenv.var.sxgaoutputheight)    
     fi          
    ;;   
    *)
    outputmode=720p
